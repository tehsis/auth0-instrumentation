...
module.exports = {
  express: function(tracer) {
    const mw = (req, res, next) => {
      const mainSpan = createRequestSpan(tracer, req);

      // Include the trace context in response headers, to facilitate debugging.
      const responseHeaders = {};
      tracer.inject(mainSpan, tracer.FORMAT_TEXT_MAP, responseHeaders);
      ...
      onFinished(res, (err, res) => {
        req.a0trace.subSpans.forEach((span) => {
          if (err || span.finishedBy === 'error') {
            span.addTags({
              [tracer.Tags.ERROR]: true,
              [tracer.Tags.SAMPLING_PRIORITY]: 1,
              'finished.by': span.finishedBy || 'error'
            });
          } else {
            span.addTags({ 'finished.by': span.finishedBy || 'response' });
          }
        ...
        if (res) {
          mainSpan.setTag(tracer.Tags.HTTP_STATUS_CODE, res.statusCode);
        }
        if (err || (res && res.statusCode >= 500)) {
          mainSpan.setTag(tracer.Tags.ERROR, true);
          mainSpan.setTag(tracer.Tags.SAMPLING_PRIORITY, 1);
        }
        mainSpan.finish();

        req.a0trace.subSpans = null;
        req.a0trace.span = null;
      });
    ...
    mw.wrap = (name, target) => {
      const wrapped = (req, res, next) => {
        ...
        req.a0trace.subSpans.add(span);

        const wrappedNext = function wrappedNext(err) {
          span.finishedBy = 'next';
          return next(err);
        };
